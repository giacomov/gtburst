...
class GUI(object):
    ...
    def _findOtherFiles(self, cspecFile, parent):
        ...
        if (prefix == "gll"):
            ...
            try:
                trigdat = pyfits.open(_getLatestVersion(
                    os.path.join(directory, "glg_trigdat_all_%s.fit" % "_".join(rootName.split("_")[1:]))))
        ...
        try:
            ...
            showerror("Error",
                      "File %s do not follow naming standards. Use the function 'Load custom dataset' from the menu file instead." % (
            ...
            try:
                ...
                if (dataHandling.testIfExecutableExists("addrmf") == None):
                    ...
                    dataset['rspfile'] = _getLatestVersion(
                        os.path.join(directory, "%s_cspec_%s.rsp2" % (prefix, rootName)))
            except:
                try:
                    dataset['rspfile'] = _getLatestVersion(
    ...
    def loadCustomDataset(self):
        ...
        customWindow.bottomtext.insert(1.0,
                                       "Warning: no check whatsoever will be performed on the files, be careful!\n\nPlease choose your data files.")
        ...
        try:
            ...
            if ('TRIGTIME' in f[0].header.keys()):
                ...
                showerror("Error in the trigger time",
                          "Could not understand the trigger time. You did not specify the MET and it is not in the event file.",
                          parent=parent)
                f.close()
                return
            pass
    ...
    def makeNavigationPlots(self):
        ...
        if (len(LATdataset) != 0):
            ...
            if (len(LLEdataset) == 0):
                showerror("No LAT data",
                          "You need to load either LAT standard data or LLE data to produce navigation plots",
                          parent=self.root)
                return
            else:
                # We have LLE data
    ...
    def simulateObservation(self):
        ...
        gtdosimulation.definedParameters['outdir'].value = os.path.join(self.configuration.get('dataRepository'),
                                                                        triggernamesim)
    ...
    def commandInterface(self):
        ...
        if (len(datasets) == 0):
            showinfo("No suitable datasets",
                     "No suitable datasets for spectral analysis. You have to load either GBM or LAT/LLE data.",
                     parent=self.root)
            return
    ...
    def writeXspecScript(self, datasetsFilter=lambda x: True):
        ...
        self.updateRootStatusbar(
            "The scripts loadData_%s_int*.xcm have been produced." % (self.datasets[0].triggerName,),
    ...
    def downloadDataFGLsource(self):
        ...
        thisWindow.bottomtext.insert(END, "FSSC website", thisWindow.hyperlinkManager.add(
            lambda: self.openWebLink('http://fermi.gsfc.nasa.gov/ssc/data/access/lat/4yr_catalog/')))
    ...
    def downloadDataSet(self):
        ...
        thisWindow.hyperlinkManager = HyperlinkManager(thisWindow.bottomtext)
        thisWindow.bottomtext.insert(END,
                                     "You can also download trigger data directly from the HEASARC clicking on these links:  ")
        thisWindow.bottomtext.insert(END, "GBM data", thisWindow.hyperlinkManager.add(lambda: self.openWebLink(
            'http://heasarc.gsfc.nasa.gov/db-perl/W3Browse/w3table.pl?tablehead=name%3Dfermigtrig&Action=More+Options')))
        thisWindow.bottomtext.insert(END, ", ")
        thisWindow.bottomtext.insert(END, "LAT LLE data", thisWindow.hyperlinkManager.add(lambda: self.openWebLink(
            'http://heasarc.gsfc.nasa.gov/db-perl/W3Browse/w3table.pl?tablehead=name%3Dfermille&Action=More+Options')))
        thisWindow.bottomtext.insert(END, ". Moreover, you can download GBM daily data ")
        thisWindow.bottomtext.insert(END, "here", thisWindow.hyperlinkManager.add(lambda: self.openWebLink(
            'http://heasarc.gsfc.nasa.gov/db-perl/W3Browse/w3table.pl?tablehead=name%3Dfermigdays&Action=More+Options')))
        thisWindow.bottomtext.insert(END, " and LAT photon data ")
        thisWindow.bottomtext.insert(END, "here", thisWindow.hyperlinkManager.add(
            lambda: self.openWebLink('http://fermi.gsfc.nasa.gov/cgi-bin/ssc/LAT/LATDataQuery.cgi')))
        thisWindow.bottomtext.insert(END, ".\n\n")
        thisWindow.bottomtext.image_create(END, image=self.lightbulb)
        thisWindow.bottomtext.insert(END,
                                     "Insert manually the trigger number, or click on 'Browse triggers' to download the list of all triggers from the HEASARC website and select from there.")
    ...
    def downloadDataSetFromFTP(self, triggerName, triggerTime, ra, dec, thisWindow, downloadLLE, downloadGBM,
        ...
        if (downloadTransient == 1):
            LATdownloader = downloadTransientData.DownloadTransientData(triggerName,
                                                                        self.configuration.get('ftpWebsite'),
                                                                        self.configuration.get('dataRepository'),
                                                                        parent=self.root)
            try:
                howManySeconds = askfloat("LAT data",
                                          "Please indicate how many seconds of data\nafter the trigger or reference date you would like to download:",
                                          initialvalue=10000, parent=self.root)
                ...
                LATdownloader.setCuts(ra, dec, 60.0, float(triggerTime), float(triggerTime) - 5000,
                                      float(triggerTime) + howManySeconds, 'MET')
            except:
                showerror("Error downloading data from FTP",
                          "Could not download data for trigger %s. Reason:\n\n '%s' \n\n." % (
    ...
    def loadDataSetsFromAdirectory(self, directory=None):
        ...
        datasetsWindow.bottomtext.insert(1.0,
                                         "In parenthesis you can find the angle between each detector and the source.\n\n")
        ...
        useOnlyCSPEC = IntVar()
        onlyCSPECbutton = Checkbutton(datasetsWindow.frame,
                                      text="Do NOT use GBM TTE files (you loose time resolution!)",
        ...
        okButton = Button(datasetsWindow.frame, text="Ok", font=NORMALFONT,
                          command=lambda: self.registerDatasets(datasets, parValues, datasetsWindow.window,
                                                                bool(useOnlyCSPEC.get())))
            ...
            if (yesNo == 1):
                ...
                if (dataset.status == "noRESP"):
                    showinfo("No response for %s" % (dataset.detector),
                             ...
                             parent=parent)
                    userResponse = fancyFileDialogs.chooseFile(
                ...
                if (dataset.status == "noTTE" and useOnlyCSPEC == False):
                    showinfo("No TTE file for detector %s" % (dataset.detector),
    ...
    def writeDefaultHelpMessage(self):
        ...
        self.updateRootStatusbar(message,
                                 "You can zoom/pan the light curve using the toolbar at the bottom of the figure. For help on the use of the toolbar, see ")
        self.bottomtext.config(state=NORMAL)
        self.bottomtext.insert(END, "http://matplotlib.org/users/navigation_toolbar.html", self.hyperlink.add(
            lambda: self.openWebLink("http://matplotlib.org/users/navigation_toolbar.html")))
        self.bottomtext.config(state=DISABLED)
