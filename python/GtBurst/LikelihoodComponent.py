...
class SourceStruct(object):
    def __init__(self, name, srctype, ra, dec, flux=0, fluxError=0, photonFlux=0, photonFluxError=0, photonIndex=0,
                 photonIndexError=0, TS=0):
        self.name = name
        self.type = srctype
        self.ra = float(ra)
        self.dec = float(dec)
    ...
    def niceXMLprint(self, inputxmlmodel, tsmin=20, phIndexForUL=-2.05):
        ...
        for source in allSources:
            ...
            if (math.ceil(TS) >= tsmin or sourceName.find('3FGL') >= 0 or source.get('type') != 'PointSource'):
                ...
                try:
                    fluxError = '%10.3g' % (
                    self.likelihoodObj.energyFluxError(sourceName, self.emin, self.emax) * MeVtoErg)
                except:
                    fluxError = 'n.a.'
